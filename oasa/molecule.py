...
class Molecule(graph.Graph):
    ...
    def mark_aromatic_bonds(self):
        ...
        while solved:
            solved = []
            for aring in rings:
                els = [self._get_atoms_possible_aromatic_electrons(
                    a, aring) for a in aring]
                if () in els:
                    continue
    ...
    def localize_aromatic_bonds(self):
        ...
        ring_clusters = list(
            map(list, [sub for sub in self.get_connected_components() if len(sub) > 1]))
        ...
        for cluster in ring_clusters:
            els = [self._get_atoms_possible_aromatic_electrons(
                a, cluster) for a in cluster]
            if () in els:
                # misuse of aromatic bonds (e.g. by smiles) or e.g. tetrahydronaphtalene
                continue
            ...
            combinations.sort(reverse=True, key=lambda x: sum(x) % 4 == 2 and 1000000+1000*len(
                [v for v in x if v == 1])+sum(x) or sum(x)+1000*len([v for v in x if v == 1]))
                    ...
                    for b in current_bonds:
                        ...
                        if v1.properties_['arom_els'] + v2.properties_['arom_els'] != 2 or \
                           (b.order in (1, 4) and (not v2.get_highest_possible_free_valency() or not v1.get_highest_possible_free_valency())):
                            ...
                            to_process.append(b)
                    to_process_atoms = self.edge_subgraph_to_vertex_subgraph(
                        to_process)
                    processed = [
                        v for v in cluster if not v in to_process_atoms]
                    # now we use the maximum_matching graph algorithm to localize the bonds
                    work_graph = self.get_induced_copy_subgraph_from_vertices_and_edges(
                        to_process_atoms, to_process, add_back_links=True)
                    # return work_graph
    ...
    def localize_aromatic_bonds_old(self):
        ...
        # sort rings
        rings.sort(lambda x, y: len(y) % 2 - len(x) %
                   2)  # odd size rings first
        last_rings = []
        while rings:
    ...
    def _read_file(self, name="mol.graph"):
        ...
        with open(name, 'r') as f:
            ...
            for l in f.readlines():
                ...
                e = self.create_edge()
                e.order = int(o)
                self.add_edge(self.vertices[int(a)],
                              self.vertices[int(b)], e=e)
    ...
    def clean_after_search(self, other):
        ...
        hs = [v for v in self.vertices if 'implicit_hydrogen' in list(
            v.properties_.keys())]
        for v in hs:
        ...
        hs = [v for v in other.vertices if 'implicit_hydrogen' in list(
            v.properties_.keys())]
        for v in hs:
            del v.properties_['implicit_hydrogen']
    ...
    def _mark_matching_threads(self, v, other):
        ...
        while threads:
            ...
            threads = [i for i in list(
                v.properties_['subsearch'].keys()) if i >= thread]
    ...
    def _spawn_thread(self, other, thread, number):
        ...
        other_vs = [
            v for v in other.vertices if thread in v.properties_['subsearch']]
        other_es = [
            e for e in other.edges if thread in e.properties_['subsearch']]
        max_thread = max([max(v.properties_['subsearch'].keys())
                          for v in other.vertices if v.properties_['subsearch']])
        for i in range(max_thread + 1, max_thread + number + 1, 1):
            for v in my_vs + my_es + other_vs + other_es:
                v.properties_['subsearch'][i] = v.properties_[
                    'subsearch'][thread]
    ...
    def _freesites_match(self, other, thread):
        for v in other.vertices:
            mirror = v.properties_['subsearch'][thread]
            unmatched_ns = [n for n in mirror.neighbors if thread not in n.properties_[
                'subsearch']]  # and not n.symbol == 'H']
        ...
        for path in double_paths:
            ...
            for e1, n1 in end1.get_neighbor_edge_pairs():
                plane1 = geometry.plane_normal_from_3_points(
                    (n1.x, n1.y, n1.z), (end1.x, end1.y, end1.z), (end2.x, end2.y, end2.z))
                if plane1 is None:
                    ...
                    for e2, n2 in end2.get_neighbor_edge_pairs():
                        if not e2 in path:
                            plane2 = geometry.plane_normal_from_3_points(
                                (end1.x, end1.y, end1.z), (end2.x, end2.y, end2.z), (n2.x, n2.y, n2.z))
    ...
    def mark_morgan(self):
        old_morgs = 0
        for v in self.vertices:
            # sum( [n.degree for n in v.neighbors])
            v.properties_['new_morgan'] = v.degree
        new_morgs = len(set([v.properties_['new_morgan']
                             for v in self.vertices]))
        ...
        while new_morgs > old_morgs:
            ...
            for v in self.vertices:
                v.properties_['new_morgan'] = v.properties_[
                    'morgan'] + sum([n.properties_['morgan'] for n in v.neighbors])
            old_morgs = new_morgs
            new_morgs = len(set([v.properties_['new_morgan']
                                 for v in self.vertices]))

        print("morgan", old_morgs, [v.properties_[
              'morgan'] for v in self.vertices])
