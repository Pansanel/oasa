...
class PybelConverter(object):
    ...
    def pybel_to_oasa_molecule(self, *args):
        omol, patom_idx2oatom = self.pybel_to_oasa_molecule_with_atom_map(
            *args)
        return omol
    ...
    def oasa_to_pybel_molecule(self, *args):
        omol, patom_idx2oatom = self.oasa_to_pybel_molecule_with_atom_map(
            *args)
        return omol
...
class ForceFieldOptimizer(object):
    ...
    def __init__(self, mol, forcefield_name="Ghemical"):
        self.mol = mol
        self._amol, self._oatom2patom_idx = PybelConverter.oasa_to_pybel_molecule_with_atom_map(
            self.mol)
        self._patom_idx2oatom = dict(
            (v, k) for k, v in list(self._oatom2patom_idx.items()))
