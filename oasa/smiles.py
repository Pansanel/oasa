...
class Smiles(plugin.Plugin):
    ...
    smiles_to_oasa_bond_recode = {'-': 1, '=': 2,
                                  '#': 3, ':': 4, ".": 0, "\\": 1, "/": 1}
    ...
    def recode_oasa_to_smiles_bond(self, b):
        if b.aromatic:
            ...
            others = [(e, st) for e, st in self._stereo_bonds_to_others[b]
            ...
            if not others:
                ...
                last_order = self._processed_atoms.index(
                    v2) - self._processed_atoms.index(v1)
    ...
    def _process_stereochemistry(self, mol):
        ...
        def get_stereobond_direction(end_atom, inside_atom, bond, init):
            position = mol.vertices.index(
                end_atom) - mol.vertices.index(inside_atom)
            ...
            if 'stereo' in v.properties_:
                ...
                if len(idx) < 3:
                    ...
                    if v.explicit_hydrogens == 0:
                        ...
                        refs = [mol.vertices[i] for i in idx1] + \
                            [h] + [mol.vertices[i] for i in idx2]
                elif len(idx) == 4:
    ...
    def get_smiles(self, mol):
        ...
        for v, st in list(self._stereo_centers.items()):
            ...
            ret = ret.replace("{{stereo%d}}" %
                              mol.vertices.index(v), ch_symbol)
        return ret
    ...
    def _get_smiles(self, mol, start_from=None):
        ...
        while not (is_line(mol) and (not start_from or start_from.degree <= 1)):
            if is_pure_ring(mol):
                if start_from:
                    self.ring_joins.append(mol.temporarily_disconnect_edge(
                        start_from.neighbor_edges[0]))
                else:
    ...
    def disconnect_something(self, mol, start_from=None):
        ...
        for e in mol.edges:
            ...
            if d1 > 2 or d2 > 2:
                ...
                ring_joints = len(
                    [i for i in the_branch if i in ring_join_vertices])
        ...
        if the_right_edge:
            ...
            the_right_mol = mol.get_induced_subgraph_from_vertices(
                the_right_mol)
            the_right_branch = mol.get_induced_subgraph_from_vertices(
                the_right_branch)
        ...
        for e in mol.edges:
            ...
            if (d1 > 2 or d2 > 2):
                ...
                if len(ps) == 1:
                    ...
                    return e, p1, v, p2
        print(mol, mol.is_connected(), ',', list(
            map(len, mol.get_connected_components())), ',', start_from)
        raise Exception("fuck, how comes!?")
...
def is_pure_ring(mol):
    return list([x for x in mol.vertices if x.degree != 2]) == []
