...
class linear_formula(object):
    ...
    def __init__(self, text="", start_valency=0, end_valency=0, mol=None):
        ...
        self.molecule = None
        if text:
            self.parse_text(text, start_valency=start_valency,
                            end_valency=end_valency, mol=mol)

    def parse_text(self, text, start_valency=0, end_valency=0, mol=None):
        text = self.expand_abbrevs(text)
        mol = self.parse_form(
            text, start_valency=start_valency, mol=mol, reverse=False)
        if mol:
            # are there any atoms?
            if not mol.vertices:
                return None
    ...
    def parse_form(self, text, start_valency=0, mol=None, reverse=False):
        ...
        if "(" not in form:
            ...
            for chunk in chunks:
                if chunk:
                    ...
                    for a in atms:
                        mol.add_vertex(a)
                        if last_atom:
                            max_val = min(last_atom.free_valency,
                                          a.free_valency, 3)
            ...
            for chunk, count in gen_formula_fragments(form, reverse=reverse):
                if chunk:
                    last_atom = self.get_last_free_atom(mol)
                    # should we string the fragments rather than adding them all to the last atom
                    do_linear = False
                    ...
                    for j in range(count):
                        if chunk[0] == "!":
                            ...
                            hs = [
                                v for v in m.vertices[0].neighbors if v.symbol == 'H']
    ...
    def expand_abbrevs(self, text):
        ...
        # (MMTr and not Tr)
        keys = sorted([(len(k), k)
                       for k in list(name_to_smiles.keys())], reverse=True)
        for l, key in keys:
