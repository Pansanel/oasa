...
class Config(object):
    database_file = os.path.abspath(os.path.join(
        os.path.dirname(__file__), "structures.db"))
...
def fill_database(infilename, name_cutoff=26, atom_count_cutoff=100):
    ...
    for line in f:
        ...
        if len(values) != 4:
            ...
            continue
        cid, inchikey, smiles, name = [x.strip()
        ...
        if len(name) <= name_cutoff and len([x for x in smiles if x.isupper()]) <= atom_count_cutoff and _allow_molecule(name, smiles):
            c.execute("INSERT INTO structures (id,name,inchikey,smiles) VALUES (?,?,?,?);",
...
def get_compounds_from_database(database_file=None, **kw):
    ...
    if search:
        sql = "SELECT structures.* FROM %s WHERE %s" % (
...
def _allow_molecule(name, smile):
    ...
    if smile.count(".") > 2:
        return False
    disallowed = ["Tl", "U", "Ra", "Rn", "At", "Po", "Bi", "Ta", "W", "Hf", "La", "Y", "Sr", "Nb", "Zr", "Eu", "Lu", "Ho", "Ce", "Nd", "Dy",
                  "Th", "Gd", "Yb", "Sm", 'Es', 'Np', 'Pu', 'Pr', 'Md', 'Pm', 'Pa', 'Tb', 'Bk', 'Cm', 'Er', 'Cf', 'Fm', 'No', "Am", "Ac", "Tm", "Lr", "Fr"]
...
def add_synonyms(fname, only_first=3):
    ...
    for line in f:
        ...
        if exists and count <= only_first:
            name = " ".join(parts[1:])
            c.execute(
                "INSERT INTO synonyms (id,synonym) VALUES (?,?);", (cid, name))
            i += 1
            if i % 500 == 0:
                connection.commit()
            if i % 10000 == 0:
...
if __name__ == "__main__":
    ...
    op = OptionParser(
        usage="python %prog -c command [options] [structure file]")
    ...
    if options.command == "test":
        print(get_compounds_from_database(
