...
def map_functions(funcs, value): return list(
    zip(apply, funcs, len(funcs)*[value]))


def something_true(vals): return len([_f for _f in vals if _f])


def some_apply(func, vals): return something_true(list(map(func, vals)))


# some results

def isinstance_of_one(obj, parents): return some_apply(
