...
def isNotEmptyText(element):
    ...
    if element.nodeValue and empty.match(element.nodeValue):
        return 0
    else:
        return 1
...
def getFirstChildNamed(element, name):
    l = [o for o in element.childNodes if (
        not o.nodeValue) and (o.localName == name)]
    if l:
        return l[0]
    else:
        return None
...
def simpleXPathSearch(element, path):
    ...
    for atomic_path in atomic_paths:
        def search_with_path(x): return _atomicXPathSearch(x, atomic_path)
