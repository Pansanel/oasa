...
class Graph(object):
    ...
    def add_vertex(self, v=None):
        ...
        if v not in self.vertices:
            ...
            warnings.warn("Added vertex is already present in graph %s" %
                          str(v), UserWarning, 2)
            return None
        self._flush_cache()
        return v
    ...
    def get_smallest_independent_cycles_e(self):
        ...
        if ncycles < 0:
            warnings.warn(
                "The number of edges is smaller than number of vertices-1, the molecule must be disconnected, which means there is something wrong with it.", UserWarning, 3)
            ncycles = 0
        ...
        while vs and len(cycles) < ncycles:
            ...
            if removed_e:
                ...
                for cycle in new_cycles:
                    ...
                    paths = set()
                    to_go = set(
                        [v for v in self.edge_subgraph_to_vertex_subgraph(cycle) if v.degree == 2])
                    while to_go:
                        now = set([to_go.pop()])
                        path = set(now)
                        while now:
        ...
        if len(cycles) < ncycles:
            warnings.warn("The number of cycles found (%d) is smaller than the theoretical value %d (|E|-|V|+1)" %
                          (len(cycles), ncycles), UserWarning, 2)
        elif len(cycles) > ncycles:
            warnings.warn("The number of independent cycles found (%d) is larger than the theoretical value %d (|E|-|V|+1), but I cannot improve it." %
                          (len(cycles), ncycles), UserWarning, 2)
    ...
    def _get_smallest_cycle_for_vertex(self, v, to_reach=None, came_from=None, went_through=None):
        ...
        for e, neigh in v.get_neighbor_edge_pairs():
            if (not went_through or neigh not in went_through) and not e == came_from:
                gens.append(self._get_smallest_cycle_for_vertex(
    ...
    def _get_smallest_cycles_for_vertex(self, v, to_reach=None, came_from=None, went_through=None):
        ...
        for e, neigh in v.get_neighbor_edge_pairs():
            ...
            if (not went_through or neigh not in went_through) and not e == came_from:
                gens.append(self._get_smallest_cycles_for_vertex(
                    neigh, to_reach=to_reach, came_from=e, went_through=w))
    ...
    def get_path_between_edges(self, e1, e2):
        ...
        if "dist" not in e2.properties_:
            ...
            for i in range(e2.properties_['dist']-1, -1, -1):
                _e = [ee for ee in _e.neighbor_edges if ee.properties_[
                    'dist'] == i][0]
                path.append(_e)
            return path
    ...
    def find_path_between(self, start, end, dont_go_through=[]):
        ...
        for i in range(d, 0, -1):
            vs = [v for e, v in rend.get_neighbor_edge_pairs() if v.properties_[
                'd'] == i-1 and (v not in dont_go_through or e not in dont_go_through)]
            if not vs:
                return None
            v = vs[0]
            out.append(v)
        ...
        while new_layer and not hit:
            ...
            for copy_parent in new_layer:
                if inner:
                    ...
                    path_from_root_to_copy_parent_original = [x.properties_[
                        'original'] for x in alt_tree.find_path_between(copy_root, copy_parent)]
