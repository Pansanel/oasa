...
class coords_optimizer(object):
    ...
    def get_angle_gradient2(self, opt_angle, refv, v1, v2):
        ...
        gx1 = (cos(ang1 - sign*dang) - cos(ang1)) * sqrt((refv.x-v1.x) **
                                                         2 + (refv.y-v1.y)**2)  # self.bond_length #(v1.x-refv.x)
        gy1 = (sin(ang1 - sign*dang) - sin(ang1)) * sqrt((refv.x-v1.x) **
                                                         2 + (refv.y-v1.y)**2)  # self.bond_length #(v1.y-refv.y)

        gx2 = (cos(ang2 + sign*dang) - cos(ang2)) * sqrt((refv.x-v2.x) **
                                                         2 + (refv.y-v2.y)**2)  # self.bond_length #(v2.x-refv.x)
        gy2 = (sin(ang2 + sign*dang) - sin(ang2)) * sqrt((refv.x-v2.x) **
                                                         2 + (refv.y-v2.y)**2)  # self.bond_length #(v2.y-refv.y)
...
if __name__ == "__main__":
    ...
    print("converged:", ok)
    print("%d iterations, RMS grad %f, max grad %f" %
          (opt.i, opt.rms_grad, opt.max_grad))
