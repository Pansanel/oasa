...
class inchi(plugin):
    ...
    def _read_inchi(self, text):
        ...
        while repeat and not self._no_possibility_to_improve:
            ...
            if not fvs and not list([_f for _f in [not v.order for v in self.structure.edges] if _f]):
                repeat = False
            else:
                if len(fvs) == 1:
                    a = fvs[0]
                    ...
                    # deal with CO and other that require adding both + and -
                        ...
                        neighbor = [n for n in a.neighbors if n.free_valency ==
                                    0 and n.symbol in self.electron_donors][0]
                        a.charge -= 1
                        neighbor.charge += 1
...
#    print("run:", run, file=sys.stderr)
    ...
    def read_charge_layer(self):
        ...
        if free_valencies % 2:
            ...
            while charge and change:
                ...
                for v in self.structure.vertices:
                    if v.symbol != 'C' and not v.free_valency and list([_f for _f in [n.free_valency for n in v.neighbors] if _f]):
                        v.charge = charge
                        charge = 0
                        change = True
                        break
    ...
    def read_p_layer(self):
        ...
        if self._protonation_dealt_with_already:
            # p was solved above
            dp = self._protonation_dealt_with_already - \
                sum([v.charge for v in self.structure.vertices])
            if dp > 0:
            ...
            for chunk in self.structure._gen_free_valency_connected_components():
                if len(chunk) % 2:
                    if len(chunk) == 1:
                        ...
                        vs = [v for v in chunk if len(
                            set(v.neighbors) & chunk) == 1]
                    for v in vs:
                        if charge < 0:
...
if __name__ == '__main__':
    ...
    def main(text, cycles):
        ...
        for jj in range(cycles):
            ...
            print("  mw:     ", mol.weight)
        print(generate_inchi_and_inchikey(
            mol, fixed_hs=False, program="/home/beda/bin/stdinchi-1"))
        t1 = time.time() - t1
