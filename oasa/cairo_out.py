...
class cairo_out(object):
    ...
    def _draw_edge(self, e):
        def draw_plain_or_colored_line(_start, _end, second=False):
            ...
            if not has_shown_vertex or not self.color_bonds:
                if not second:
                    self._draw_line(
                        _start, _end, line_width=self.line_width, capstyle=cairo.LINE_CAP_ROUND)
                else:
                    self._draw_line(
                        _start, _end, line_width=self.line_width, capstyle=cairo.LINE_CAP_BUTT)
            else:
                self._draw_colored_line(
                    _start, _end, line_width=self.line_width, start_color=color1, end_color=color2)
        ...
        def draw_plain_or_colored_wedge(_start, _end):
            ...
            if not has_shown_vertex or not self.color_bonds:
                self._create_cairo_path(
                    [(xa, ya), (x0, y0), (2*x2-x0, 2*y2-y0), (2*x1-xa, 2*y1-ya)], closed=True)
                self.context.set_source_rgb(0, 0, 0)
                self.context.fill()
            else:
                ...
                self._create_cairo_path(
                    [(xa, ya), (xm1, ym1), (xm2, ym2), (2*x1-xa, 2*y1-ya)], closed=True)
                self.context.fill()
                self.context.set_source_rgb(*color2)
                self._create_cairo_path(
                    [(xm1, ym1), (x0, y0), (2*x2-x0, 2*y2-y0), (xm2, ym2)], closed=True)
                self.context.fill()
        ...
        def draw_plain_or_colored_hatch(_start, _end):
            ...
            self.context.set_source_rgb(*color1)
            middle = 0.5 * \
                (draw_start + int(round(d / step_size)) + draw_end - 2)
            for i in range(draw_start, int(round(d / step_size)) + draw_end):
                coords = [xa+dx1*i*step_size, ya+dy1*i*step_size,
                          2*x1-xa+dx2*i*step_size, 2*y1-ya+dy2*i*step_size]
            ...
            if e.order == 2:
                ...
                if side:
                    ...
                    x1, y1, x2, y2 = geometry.find_parallel(
                        start[0], start[1], end[0], end[1], self.bond_width*misc.signum(side))
    ...
    def _find_place_around_atom(self, atom):
        ...
        angles = [geometry.clockwise_angle_from_east(
            x1-x, y1-y) for x1, y1 in coords]
    ...
    def _draw_vertex(self, v):
        pos = sum(
            [(a.x < v.x) and -1 or 1 for a in v.neighbors if abs(a.x-v.x) > 0.2])
        if 'show_symbol' in v.properties_:
            show_symbol = v.properties_['show_symbol']
        else:
            show_symbol = (v.symbol != "C" or v.degree ==
                           0 or self.show_carbon_symbol)
        if show_symbol:
            x = v.x
            y = v.y
            text = v.symbol
            ...
            self._vertex_to_bbox[v] = (
                (x, y), geometry.expand_rectangle(bbox, self.space_around_atom))
            ...
            if charge:
                ...
                self.context.set_font_size(
                    self.subscript_size_ratio * self.font_size)
                ...
                line = (v.x, v.y, x0, y0)
                charge_bbox = [x0-0.5*width, y0-0.5 *
                               height, x0+0.5*width, y0+0.5*height]
                x1, y1 = geometry.intersection_of_line_and_rect(
                    line, bbox, round_edges=0)
                x2, y2 = geometry.intersection_of_line_and_rect(
                    line, charge_bbox, round_edges=0)
                x2, y2 = geometry.elongate_line(
                    x1, y1, x2, y2, -self.space_around_atom)
                x = x0 + x1 - x2
                y = y0 + y1 - y2
                # draw
    ...
    def _draw_text(self, xy, text, font_name=None, font_size=None, center_letter=None,
            ...
            def _closeCurrentText(self):
                if self._text:
                    self.chunks.append(text_chunk(
                        self._text, attrs=set(self._above)))
                    self._text = ""
        ...
        for chunk in chunks:
            ...
            if self.add_background_to_text:
                ...
                # self.context.stroke()
            # text itself
            _bbox = [x1+xbearing, y1+ybearing, x1 +
                     xbearing+width, y1+ybearing+height]
    ...
    def _create_cairo_path(self, points, closed=False):
        points = [self._invtransform.transform_xyz(
            p[0], p[1], 0)[:2] for p in points]
        x, y = points[0]
