...
class coords_generator(object):
    ...
    def calculate_coords(self, mol, bond_length=0, force=0):
        ...
        if len(atms) == len(mol.vertices) and not force:
            ...
            if mol.contains_cycle():
                ...
                gcoords = gen_ring_coords(
                    len(backbone), side_length=self.bond_length)
            ...
            for v in processed:
                ...
                if not ring:
                    ...
                    if len(processed) > 1 and mol.defines_connected_subgraph_v(processed) and set(processed) <= ring:
                        new_processed += self.process_all_anelated_rings(
                            processed)
                    else:
                        self.rings.remove(ring)
                        ...
                        ca = geometry.clockwise_angle_from_east(
                            v.x-d.x, v.y-d.y)
                        size = len(ring)+1
        ...
        if len(done) == 1 and (len(to_go) == 1 or len(to_go) == 2 and [1 for _t in to_go if _t in self.stereo]):
            ...
            if t in self.stereo:
                ...
                if ss:
                    ...
                    angle_to_add = get_angle_at_side(
                        v, d, d2, relation, angle_to_add)
                    placed = True
            ...
            angles = [geometry.clockwise_angle_from_east(
                at.x-v.x, at.y-v.y) for at in done]
    ...
    def _process_simply_anelated_ring(self, ring, base):
        ...
        if len(inter) == 1:
            ...
            if not v1 in ring[0].neighbors:
                v1, v2 = v2, v1
            side = sum([geometry.on_which_side_is_point(
                (v1.x, v1.y, v2.x, v2.y), (v.x, v.y)) for v in base])
            if not side:
    ...
    def _process_multi_anelated_ring(self, ring, angle_shift=0):
        ...
        if not sorted_back:
            ...
            ca = deg_to_rad(180-(overall_angle - blocked_angle -
                                 len(to_go) * da + angle_shift)/2)  # connection angle
            side = sum([geometry.on_which_side_is_point(
                (v1.x, v1.y, v2.x, v2.y), (v.x, v.y)) for v in back if v != v1 and v != v2])
...
def show_mol(mol):
    ...
    paper = Canvas(mainFrame, width=640, height=480,
                   background="white", closeenough=5)
    paper.pack()
    ...
    def xtrans(xx): return range*(xx - xmin)+xp/2

    def ytrans(xx): return range*(xx - ymin)+yp/2
