...
def angle_between_lines(l1, l2):
    ...
    a = clockwise_angle_from_east(
        l1[2]-l1[0], l1[3]-l1[1]) - clockwise_angle_from_east(l2[2]-l2[0], l2[3]-l2[1])
    if a > pi:
        return a - 2*pi
    elif a < -pi:
        return a + 2*pi
...
def tkspline_to_cubic_bezier(points):  # points = ((x1,y1),(x2,y2),...)
    ...
    for qbez in qbeziers:
        ...
        ccurvexa, ccurveya = startx + 2/3 * \
            (qcurvex-startx), starty + 2/3*(qcurvey-starty)
        ccurvexb, ccurveyb = ccurvexa + 1/3 * \
            (endx-startx), ccurveya + 1/3*(endy-starty)
        cbeziers.append((startx, starty, ccurvexa, ccurveya,
                         ccurvexb, ccurveyb, endx, endy))
...
def angle_between_planes(plane1, plane2):
    a1, b1, c1 = plane1
    a2, b2, c2 = plane2
    cos = (a1*a2 + b1*b2 + c1*c2) / sqrt(a1**2 +
                                         b1**2+c1**2) / sqrt(a2**2+b2**2+c2**2)
    return cos
